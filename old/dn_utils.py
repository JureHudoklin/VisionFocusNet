...
def prepare_for_dn(targets, #  List[Dict[str, Tensor]]
    ...
    if dn_args is not None:
        scalar, label_noise_scale, box_noise_scale, use_dn, use_indicator = dn_args["NUM_DN_GROUPS"], dn_args["LABEL_NOISE_SCALE"], dn_args["BOX_NOISE_SCALE"], dn_args["USE_DN"], dn_args["USE_INDICATOR"]
    else:
    ...
    # if dn_args["ADD_OBJECTNESS"]:
    #     objectness = torch.ones(num_queries, batch_size, 1).to(ref_tgt.device)
    #     objectness = label_enc(objectness) # [Q, B, C]
    #     ref_tgt = ref_tgt + objectness
    ...
    if training and use_dn and dn_args is not None:
        ...
        input_label_embed = label_enc(m) # [N*scalar, C]
        ...
        mask_dict = {
            ...
            'known_lbs_bboxes': (known_labels, known_bboxs), # ([N*scalar], [N*scalar, 4])
...
def tgt_loss_labels(src_logits_, tgt_labels_, num_tgt, focal_alpha, log=True):
    ...
    losses['tgt_class_error'] = accuracy(src_logits_, tgt_labels_)[0]
    return losses
