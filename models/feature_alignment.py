...
class TemplateFeatAligner_v2(nn.Module):
    def __init__(self,
                 d_model = 256,
                 n_heads = 8,):
        super().__init__()
        ...
        self.temp_deproj = nn.Sequential(
            nn.Linear(self.d_model*2, self.d_model),
            nn.PReLU(),
        )
        
        self.weight_pred_net = MLP(self.d_model, self.d_model*2, self.d_model*2, 2, activation="prelu")
    ...
    def forward(self, image_feat: Tensor, temp_feat: Tensor, ref_points: Tensor, image_sizes: Tensor) -> Tensor:
        ...
        # Align the template features
        roi_feat_align = roi.view(B, num_queries, C) # (B, Q, C)
        weights = self.weight_pred_net(roi_feat_align) # (B, Q, 2*C)
        temp_feat_proj = self.temp_proj(temp_feat) # (B, num_tgts, 2*C)
        temp_feat_proj = temp_feat_proj[:, None, :, :].repeat(1, num_queries, 1, 1) # (B, Q, num_tgts, 2*C)
        temp_feat_proj = temp_feat_proj * weights[:, :, None, :] # (B, Q, num_tgts, 2*C)
        temp_aligned = self.temp_deproj(temp_feat_proj) # (B, Q, num_tgts, C)
        
        # Select the best aligned template feature (B, Q, num_tgts, C) -> (B, Q, C)
        # Split heads
        q = self.q_lin(roi_feat_align).view(B, num_queries, self.n_heads, self.d_model // self.n_heads).permute(0, 2, 1, 3) # (B, n_heads, Q, d_model // n_heads)
        k = self.k_lin(temp_aligned).view(B, num_queries, num_tgts, self.n_heads, self.d_model // self.n_heads).permute(0, 3, 1, 2, 4) # (B, n_heads, Q, num_tgts, d_model // n_heads)
        v = temp_aligned.view(B, num_queries, num_tgts, self.n_heads, self.d_model // self.n_heads).permute(0, 3, 1, 2, 4) # (B, n_heads, Q, num_tgts, d_model // n_heads)
...
class TemplateFeatAligner_v3(nn.Module):
    def __init__(self,
                 d_model = 256,
                 n_heads = 8,):
        super().__init__()
    ...
    def forward(self, image_feat: Tensor, temp_feat: Tensor, ref_points: Tensor, image_sizes: Tensor) -> Tensor:
        ...
        out = out-roi_feat_align
        
        return out
    
    
    
class TemplateFeatAligner_v4(nn.Module):
    def __init__(self,
                 d_model = 256,
                 n_heads = 8,):
        super().__init__()
    ...
    def forward(self, query_feat: Tensor, temp_feat: Tensor) -> Tensor:
        """_summary_

        Parameters
        ----------
        query_feat : Tensor (B, Q, C)
            _description_
        temp_feat : Tensor (B, num_tgts, C)
            _description_
        ...
        # Align the template features
        roi_feat_align = query_feat # (B, Q, C)
        weights = self.weight_pred_net(roi_feat_align) # (B, Q, 2*C)
        temp_feat_proj = self.temp_proj(temp_feat) # (B, num_tgts, 2*C)
        temp_feat_proj = temp_feat_proj[:, None, :, :].repeat(1, num_queries, 1, 1) # (B, Q, num_tgts, 2*C)
        temp_feat_proj = temp_feat_proj * weights[:, :, None, :] # (B, Q, num_tgts, 2*C)
...
class TemplateFeatAligner_v5(nn.Module):
    def __init__(self,
                 d_model = 256,
                 n_heads = 8,):
        super().__init__()
